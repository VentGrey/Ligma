    ...
    my $update_wifi = sub {
        ...
        if ($output !~ /no wireless extensions/) {
            if ($output =~ /Bit Rate=([\d\.]+ Mb\/s)/) {
                my $bitrate = $1;
                return "+\@fg=1;󰤨 $bitrate+\@fg=0;";  # Nerd font icon for WiFi connected with BitRate
            }
            return "+\@fg=3;󰤭+\@fg=0;";  # Nerd font icon for WiFi connected without BitRate
        }
        return "+\@fg=3;󰤭+\@fg=0;";  # Nerd font icon for WiFi disconnected
    };
